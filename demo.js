...
function onAccionPalindromo() {
	...
	let regex=/[a-z0-9]/gi;
	let cadenaP=str.match(regex).join("");
	cadenaP=cadenaP.toLowerCase();
	let nuevaCadena="";
	for (let x = cadenaP.length - 1; x >= 0; x--) {
...
function scr_demo_palindromo(my) {
	my.state = {
		Mensaje: '\n\n\ningrese la palabra para saber\n\n sí es un palindromo\n\n_Chau!_',
	};

	my.render = function scr_palindromo_render() {
		return [
			{ cmp: 'PaMenuYCerrar' },
			my.forValue('Mensaje', { cmp: 'TextArea', style: { display: 'block', width: '100%' }, rows: 5 }),
			{ cmp: 'Button', href: demo_palindrome(my.state.Mensaje)}
		];
	}
}
